...
func load_step(step_number):
    ...
    fade_in.interpolate_property(node, "modulate", initial_value, final_value, 0.75, Tween.TRANS_LINEAR, 0, 0.3)
    fade_in.start()
...
func complete_step(step_number):
    ...
    fade_out.interpolate_property(node, "modulate", initial_value, final_value, 0.75, Tween.TRANS_LINEAR, 0)
    fade_out.start()
